...
namespace {
// Exponential moving average for battery current: alpha = 1 - exp(-1/10) for a
// ~10 s time constant when BATTERY_STATUS streams at ~1 Hz.
constexpr double kBattCurrentEmaAlpha = 0.095;
// BATTERY_STATUS stale threshold in milliseconds.
...
void TelemetryBridge::setVehicle(Vehicle* vehicle) {
    ...
    // Battery data is per-vehicle; reset SOC / EMA / freshness when switching.
    _batterySocPct = -1;
    _batteryCurrentSmoothAmps = 0.0;
    _batteryDataValid = false;
    _lastBatteryStatusTime = QDateTime();
    emit batterySocPctChanged();
    emit batteryCurrentSmoothAmpsChanged();
    emit batteryDataValidChanged();
...
bool TelemetryBridge::isConnected() const { return _vehicle != nullptr; }

bool TelemetryBridge::armed() const { return _vehicle ? _vehicle->armed() : false; }

QString TelemetryBridge::flightMode() const { return _vehicle ? _vehicle->flightMode() : QString(); }
...
void TelemetryBridge::_connectVehicleSignals() {
    ...
    connect(_vehicle, &Vehicle::coordinateChanged, this, [this](QGeoCoordinate coord) {
        bool changed = false;
        if (qAbs(coord.latitude() - _gpsLatitude) > 1e-7) {
            _gpsLatitude = coord.latitude();
            changed = true;
        }
        if (qAbs(coord.longitude() - _gpsLongitude) > 1e-7) {
            _gpsLongitude = coord.longitude();
            changed = true;
        }
    ...
    connect(_vehicle->parameterManager(), &ParameterManager::parametersReadyChanged, this,
            &TelemetryBridge::_onParameterReadyChanged);
    ...
    connect(_vehicle, &Vehicle::homePositionChanged, this, [this](QGeoCoordinate home) {
        ...
        bool changed = false;
        if (qAbs(lat - _homeLatitude) > 1e-7) {
            _homeLatitude = lat;
            changed = true;
        }
        if (qAbs(lon - _homeLongitude) > 1e-7) {
            _homeLongitude = lon;
            changed = true;
        }
        if (qAbs(alt - _homeAltitude) > 0.1) {
            _homeAltitude = alt;
            changed = true;
        }
        if (changed) {
    ...
    // battery list model for dynamically-added batteries (e.g. secondary battery).
    {
        ...
        connect(battModel, &QmlObjectListModel::rowsInserted, this,
                [this]() { _connectBatteryFacts(_vehicle->batteries()); });
    }
    ...
    if (auto* missionMgr = _vehicle->missionManager()) {
        connect(missionMgr, &MissionManager::newMissionItemsAvailable, this, [this](bool) { _updateMissionInfo(); });
        connect(missionMgr, &MissionManager::currentIndexChanged, this, [this](int) { _updateMissionInfo(); });
    }
...
static QString parseBatteryChargeState(uint8_t state) {
    switch (state) {
    ...
    for (int i = 0; i < model->count(); ++i) {
        ...
        if (id == 0 || id == 1) {
            ...
            connect(batt->current(), &Fact::valueChanged, this, [this, id](QVariant val) {
                ...
                if (id == 0) {
                    _batteryCurrent = c;
                    _batteryCurrentAmps = c;
                    emit batteryCurrentChanged();
                    emit batteryCurrentAmpsChanged();
                } else {
                    _battery2Current = c;
            ...
            // Charge state (primary battery only).
            connect(batt->chargeState(), &Fact::valueChanged, this, [this, id](QVariant val) {
                if (id == 0) {
                    QString st = parseBatteryChargeState(static_cast<uint8_t>(val.toUInt()));
                    if (_batteryChargeState != st) {
                        _batteryChargeState = st;
                        emit batteryChargeStateChanged();
                    }
                }
            });
            // Time remaining (primary battery only).
            connect(batt->timeRemaining(), &Fact::valueChanged, this, [this, id](QVariant val) {
                if (id == 0) {
                    double sec = val.toDouble();
                    if (qAbs(sec - _batteryTimeRemainingSeconds) > 1.0) {
                        _batteryTimeRemainingSeconds = sec;
                        emit batteryTimeRemainingSecondsChanged();
                    }
                }
            });
            ...
            if (id == 0) {
                ...
                _batteryCurrentAmps = _batteryCurrent;
                _batteryPercent = batt->percentRemaining()->rawValue().toInt();
                _batteryTemperature = batt->temperature()->rawValue().toDouble();
                _batteryChargeState =
                    parseBatteryChargeState(static_cast<uint8_t>(batt->chargeState()->rawValue().toUInt()));
                _batteryTimeRemainingSeconds = batt->timeRemaining()->rawValue().toDouble();
            } else {
...
int TelemetryBridge::_fwMotorOutputChannel() {
    if (_fwMotorChannel > 0) return _fwMotorChannel;
...
void TelemetryBridge::_loadParameters() {
    ...
    if (!_paramConnections.isEmpty()) {
        for (auto& c : _paramConnections) disconnect(c);
    ...
    for (const QString& name : watchlistNames) {
        if (!availableParams.contains(name)) continue;
        Fact* fact = paramMgr->getParameter(compId, name);
        if (!fact) continue;
        ...
        _paramConnections.append(connect(fact, &Fact::valueChanged, this, [this, paramName]() {
            Vehicle* v = _vehicle;
            if (!v) return;
...
void TelemetryBridge::_onTelemetryUpdate() { _updateFromVehicle(); }
...
/// available at connect time) and flips battery data stale after 5 s of silence.
void TelemetryBridge::_updateFromVehicle() {
    ...
    // Stale battery data: no BATTERY_STATUS for > 5 s → invalid. Emit only on
    // the valid→invalid transition; the next BATTERY_STATUS re-arms validity.
    if (_batteryDataValid && _lastBatteryStatusTime.isValid()) {
        if (_lastBatteryStatusTime.msecsTo(QDateTime::currentDateTime()) > kBatteryStaleMs) {
            _batteryDataValid = false;
            emit batteryDataValidChanged();
        }
    }
...
void TelemetryBridge::_handleMavlinkMessage(const mavlink_message_t& message) {
    switch (message.msgid) {
        ...
        case MAVLINK_MSG_ID_SERVO_OUTPUT_RAW: {
            ...
            if (servo.port == 0) {
                uint16_t vals[16] = {servo.servo1_raw,  servo.servo2_raw,  servo.servo3_raw,  servo.servo4_raw,
        ...
        case MAVLINK_MSG_ID_HEARTBEAT: {
            ...
            if (message.compid != 1 && message.compid != 0) _companionDetected = true;
            if (wasCompanion != _companionDetected) emit companionDetectedChanged();
        ...
        case MAVLINK_MSG_ID_BATTERY_STATUS: {
            ...
            if (batt.id == 0) {
                // Temperature (int16_t in centidegrees C, INT16_MAX = invalid)
                if (batt.temperature != INT16_MAX) {
                    double tempC = batt.temperature / 100.0;
                    if (qIsNaN(_batteryTemperature) || qAbs(tempC - _batteryTemperature) > 0.1) {
                        _batteryTemperature = tempC;
                        emit batteryTemperatureChanged();
                    }
                }

                // Current (int16_t in 10mA units / centiamps, -1 = invalid)
                if (batt.current_battery >= 0) {
                    double amps = batt.current_battery / 100.0;
                    if (qAbs(amps - _batteryCurrentAmps) > 0.01) {
                        _batteryCurrentAmps = amps;
                        _batteryCurrent = amps;
                        emit batteryCurrentAmpsChanged();
                        emit batteryCurrentChanged();
                    }

                    // 10-second EMA accumulated over every message (never reset mid-stream).
                    // First valid sample primes the filter so the chip shows a value
                    // almost immediately after connect/arm.
                    if (_batteryCurrentSmoothAmps <= 0.0) {
                        _batteryCurrentSmoothAmps = amps;
                    } else {
                        _batteryCurrentSmoothAmps =
                            kBattCurrentEmaAlpha * amps + (1.0 - kBattCurrentEmaAlpha) * _batteryCurrentSmoothAmps;
                    }
                    emit batteryCurrentSmoothAmpsChanged();
                }

                // SOC: raw battery_remaining 0-100, UINT8_MAX (255) = not configured.
                // Never derived from voltage — the FC value (or absence) is authoritative.
                {
                    const int socRaw = static_cast<int>(batt.battery_remaining);
                    const int soc = (batt.battery_remaining == UINT8_MAX) ? -1 : socRaw;
                    if (soc != _batterySocPct) {
                        _batterySocPct = soc;
                        emit batterySocPctChanged();
                    }
                }

                // Mark battery data fresh when SOC is meaningful; the poll timer
                // drops it to false after 5 s of silence. When the FC does not
                // report SOC (battery not configured) data stays invalid so callers
                // can distinguish "SOC unknown" from "battery telemetry stale".
                _lastBatteryStatusTime = QDateTime::currentDateTime();
                if (batt.battery_remaining != UINT8_MAX && !_batteryDataValid) {
                    _batteryDataValid = true;
                    emit batteryDataValidChanged();
                }

                // Time remaining (int32_t seconds, <=0 = invalid)
                if (batt.time_remaining > 0) {
                    double sec = static_cast<double>(batt.time_remaining);
                    if (qAbs(sec - _batteryTimeRemainingSeconds) > 1.0) {
                        _batteryTimeRemainingSeconds = sec;
                        emit batteryTimeRemainingSecondsChanged();
                    }
                ...
                // Charge state
                QString st = parseBatteryChargeState(batt.charge_state);
                if (_batteryChargeState != st) {
                    _batteryChargeState = st;
                    emit batteryChargeStateChanged();
                }

                // Cell voltages (voltages: uint16_t[10] in mV, voltages_ext: uint16_t[4] in mV)
                QVariantList cells;
                for (int i = 0; i < 10; ++i) {
                    if (batt.voltages[i] != UINT16_MAX) {
                        cells.append(batt.voltages[i] / 1000.0);
                    }
                }
                for (int i = 0; i < 4; ++i) {
                    if (batt.voltages_ext[i] != 0 && batt.voltages_ext[i] != UINT16_MAX) {
                        cells.append(batt.voltages_ext[i] / 1000.0);
                    }
                }
                if (_batteryCellVoltages != cells) {
                    _batteryCellVoltages = cells;
                    emit batteryCellVoltagesChanged();
                }
            }
            break;
        }
...
void TelemetryBridge::_updateMissionInfo() {
    ...
    for (const auto* item : items) {
        ...
        if (firstValid) totalDist += prev.distanceTo(coord);
        prev = coord;
        firstValid = true;
    }
...
bool TelemetryBridge::hasParameter(const QString& name) const { return _parameterCache.contains(name); }
...
void TelemetryBridge::arm() {
    ...
    _vehicle->sendMavCommand(_vehicle->defaultComponentId(), MAV_CMD_COMPONENT_ARM_DISARM,
                             true,   // showError
                             1.0f);  // param1 = 1 (arm)
}

/// Store a reference to the ArmingGate so arm() can query gate state before sending.
/// Uses QObject* to avoid circular header dependency; accesses properties dynamically.
void TelemetryBridge::setArmingGate(QObject* gate) { _armingGate = gate; }

/// Force-arms the vehicle without consulting the ArmingGate or pre-arm checks.
/// Sends MAV_CMD_COMPONENT_ARM_DISARM with param1=1 (arm) and param2=21196 (ArduPilot
/// force-arm magic number accepted by both ArduPilot and PX4).
void TelemetryBridge::forceArm() {
    if (!_vehicle) {
        qCWarning(telemetryBridgeLog) << "forceArm(): no vehicle connected";
        return;
    }

    qCWarning(telemetryBridgeLog) << "forceArm(): sending MAV_CMD_COMPONENT_ARM_DISARM with force flag";
    _vehicle->sendMavCommand(_vehicle->defaultComponentId(), MAV_CMD_COMPONENT_ARM_DISARM,
                             true,       // showError
                             1.0f,       // param1 = 1 (arm)
                             2989.0f);   // param2 = force-arm magic 2989
}
